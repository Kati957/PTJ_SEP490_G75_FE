...
const AdminJSPostPage: React.FC = () => {
  ...
  const [selectedPost, setSelectedPost] = useState<AdminJobPostView | null>(
    null
  );
  ...
  const handleView = async (id: number) => {
    ...
    try {
      const postDetail = await adminService.getEmployerPostDetail(id);
  ...
  const columns: TableColumnsType<AdminJobPostView> = [
    {
      title: "ID",
      dataIndex: "id",
      key: "id",
    ...
    },
    {
      ...
      render: (name, record) => (
        <div>
          <div>{name || "(Chưa có tên)"}</div>
          <small>{record.employerEmail}</small>
        </div>
      ),
    },
    {
        ...
        {isModalLoading ? (
          ...
          <div>
            <div className="bg-blue-900 text-white px-5 py-3 rounded-t-lg -m-6 mb-4">
              ...
              <p className="text-blue-100 text-sm">Mã: {selectedPost.id}</p>
            </div>
